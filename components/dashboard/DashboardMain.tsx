...
async function fetchUserdata(user: User) {
  ...
  const weekData: WeekGraphPoint[] = Object.entries(weeklyData).map(([k, v]) => ({
    week: k,
    totalHours: Number(v) || 0,
  }));
...
export default function DashboardMain() {
  ...
  const [weeklyData, setWeeklyData] = useState<Record<WeekKey, number>>({} as Record<WeekKey, number>);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (activeTab === 'overview') {
        ...
        setOverviewData([
          { id: 'hoursLogged', title: 'Hours Logged', value: data.hoursLogged, change: '+0 today' },
          { id: 'tasksCompleted', title: 'Tasks Completed', value: data.tasksCompleted, change: '+0 today' },
        ]);
          ...
          <section className="mt-10">
            ...
            {weekData.length ? (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {weekData.map((w) => (
                  <div key={w.week} className="p-4 bg-blue-100 rounded shadow text-center">
                    <h4 className="font-medium">{w.week}</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {taskData.map((t) => (
                ...
                >
                  ...
                  <Button variant="destructive" size="sm" onClick={() => handleDeleteTask(t.id)}>
                    Delete
                  </Button>
                </div>
              ))}
